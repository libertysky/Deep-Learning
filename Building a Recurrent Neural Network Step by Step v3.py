...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    # Compute gates related derivatives, equations (7) to (10) (≈4 lines)
    ...
    da_prev = (np.dot(Wf.T,dft) +np.dot(Wi.T,dit) + np.dot(Wc.T,dcct) + np.dot(Wo.T,dot))[:n_a,:]
    dc_prev = dc_next * ft + ot * (1-np.tanh(c_next)**2) * ft * da_next
    dxt = (np.dot(Wf.T,dft) +np.dot(Wi.T,dit) + np.dot(Wc.T,dcct) + np.dot(Wo.T,dot))[n_a:,:]
